...
export default function CreateArticle() {
  ...
  const onSubmit: SubmitHandler<ArticleFormData> = (formData) => submitArticle(formData)
  ...
  const notify = () => toast('Article created successfully');
  return (
    <>
      ...
      <div data-testid="formDiv" className="flex flex-col gap-5 items-center">
        ...
        <input type="text" {...register("name", { required: "Name is required", minLength: {value:5, message: "Name should'nt be lessthan 5 characters"} })} className={`${errors.name ? 'border-red-500' : ''} focus:outline-none border px-5 py-2 w-[500px] rounded-md`} placeholder='John Doe'/>
         {errors.name && <span className='text-red-500 text-sm font-light font-nunito'>{errors.name?.message}</span>}
        </div>
        ...
        {<span className="text-red-500">*</span> }
          ...
          <input type="text" {...register("articleTitle", { required: true })} className={`${errors.articleTitle ? 'border-red-500' : ''} focus:outline-none border px-5 py-2 w-[500px] rounded-md`} placeholder='Unique title'/>
        ...
        <button className={`bg-[#606c38] text-white px-10 py-2 rounded-md hover:bg-[#283618] focus:border-rose-700 active:bg-rose-700 transition ease-in-out duration-150 ${loader ? 'cursor-not-allowed' : ''}`}>
